...
if (!class_exists('XmlExportWooCommerceOrder')) {
    final class XmlExportWooCommerceOrder
    {
            ...
            foreach (self::$order_sections as $slug => $section) :
                ...
                if (!empty($section['additional'])) {
                    foreach ($section['additional'] as $sub_slug => $sub_section) {
                        if(isset($sub_section['meta']) && is_array($sub_section['meta'])) {
        ...
        public function render(& $i)
        {
            ...
            foreach (self::$order_sections as $slug => $section) :
                if (!empty($section['meta']) or !empty($section['additional'])):
                    ...
                    <?php if ($slug != 'items' && $slug != 'taxes' && $slug != 'fees' && $slug != 'notes' && $slug != 'refunds') { ?>
                                ...
                                foreach ($section['meta'] as $cur_meta_key => $field) {
                                    ...
                                    <li class="pmxe_<?php echo esc_attr($slug); ?>  <?php if ($is_auto_field) echo 'wp_all_export_auto_generate'; ?>">
                                        <div class="custom_column" rel="<?php echo esc_attr($i + 1); ?>">
                                            <label class="wpallexport-xml-element"><?php echo esc_attr((is_array($field)) ? $field['name'] : $field); ?></label>
                                            <input type="hidden" name="ids[]" value="1"/>
                                            <input type="hidden" name="cc_label[]"
                                                   value="<?php echo esc_attr((is_array($field)) ? $field['label'] : $cur_meta_key); ?>"/>
                                            ...
                                            <input type="hidden" name="cc_options[]"
                                                   value="<?php echo esc_attr((is_array($field)) ? $field['options'] : $slug); ?>"/>
                                            <input type="hidden" name="cc_type[]"
                                                   value="<?php echo esc_attr((is_array($field)) ? $field['type'] : 'woo_order'); ?>"/>
                                            <input type="hidden" name="cc_value[]"
                                                   value="<?php echo esc_attr((is_array($field)) ? $field['label'] : $cur_meta_key); ?>"/>
                                            <input type="hidden" name="cc_name[]"
                                                   value="<?php echo esc_attr((is_array($field)) ? $field['name'] : $field); ?>"/>
                                            <input type="hidden" name="cc_settings[]" value=""/>
                                            <input type="hidden" name="cc_combine_multiple_fields[]"
                                                   value="<?php echo esc_attr((is_array($field) && isset($field['cc_combine_multiple_fields'])) ? $field['cc_combine_multiple_fields'] : ''); ?>"/>
                                            <input type="hidden" name="cc_combine_multiple_fields_value[]"
                                                   value="<?php echo esc_attr((is_array($field) && isset($field['cc_combine_multiple_fields_value'])) ? $field['cc_combine_multiple_fields_value'] : ''); ?>"/>
                                    ...
                                    <li class="available_sub_section">
                                        ...
                                        <div class="wpae-custom-field">
                                            <ul>
                                                <li>
                                                    <div class="default_column" rel="">
                                                        <label class="wpallexport-element-label"><?php echo esc_html("All" . ' ' . $sub_section['title']); ?></label>
                                                ...
                                                foreach ($sub_section['meta'] as $field) {
                                                    ?>
                                                    <li class="<?php echo esc_attr("pmxe_" . $slug); ?>_<?php echo esc_html($sub_slug); ?>">
                                                        <div class="custom_column" rel="<?php echo($i + 1); ?>">
                                                            ...
                                                            <input type="hidden" name="ids[]" value="1"/>
                                                            <input type="hidden" name="cc_label[]"
                                                                   value="<?php echo esc_attr("item_data__" . (is_array($field)) ? $field['label'] : $field); ?>"/>
                                                            <input type="hidden" name="cc_php[]" value=""/>
                                                            <input type="hidden" name="cc_code[]" value=""/>
                                                            <input type="hidden" name="cc_sql[]" value=""/>
                                                            <input type="hidden" name="cc_options[]" value="item_data"/>
                                                            <input type="hidden" name="cc_type[]"
                                                                   value="<?php echo esc_attr((is_array($field)) ? $field['type'] : $sub_slug); ?>"/>
                                                            <input type="hidden" name="cc_value[]"
                                                                   value="<?php echo esc_attr("item_data__echo" . (is_array($field)) ? $field['label'] : $field); ?>"/>
                                                            <input type="hidden" name="cc_name[]"
                                                                   value="<?php echo esc_attr((is_array($field)) ? $field['name'] : $field); ?>"/>
                                                            <input type="hidden" name="cc_settings[]" value=""/>
                                                            <input type="hidden" name="cc_combine_multiple_fields[]"
                ...
                <?php } else {
                        echo '<div class="wpae-custom-field"><div class="wpallexport-free-edition-notice">';
                        if(PMXE_EDITION === 'free') {
                            if ($slug == 'items') {
                                ?>
                            ...
                            if ($slug == 'items') {
                                ...
                                <a class="upgrade_link" target="_blank"
                                   href="https://www.wpallimport.com/portal/downloads">
                                    Purchase the WooCommerce Export Add-On Pro to Export Order Items</a>
                                <?php
                            } else if ($slug == 'taxes') {
                                ?>
                                <a class="upgrade_link" target="_blank"
                                   href="https://www.wpallimport.com/portal/downloads">
                                    Purchase the WooCommerce Export Add-On Pro to Export Taxes &amp; Shipping</a>
                                <?php
                            } else if ($slug == 'fees') {
                                ?>
                                <a class="upgrade_link" target="_blank"
                                   href="https://www.wpallimport.com/portal/downloads">
                                    Purchase the WooCommerce Export Add-On Pro to Export Fees &amp; Discounts</a>
                                <?php
                            } else if ($slug == 'notes') {
                                ?>
                                <a class="upgrade_link" target="_blank"
                                   href="https://www.wpallimport.com/portal/downloads">
                                    Purchase the WooCommerce Export Add-On Pro to Export Order Notes</a>
                                <?php
                            } else if ($slug == 'refunds') {
                                ?>
                                <a class="upgrade_link" target="_blank"
                                   href="https://www.wpallimport.com/portal/downloads">
                                    Purchase the WooCommerce Export Add-On Pro to Export Order Refunds</a>
                                <?php
                            }
                        }
        ...
        public function render_new_field()
        {
            ...
            unset(self::$order_sections['items']);
            unset(self::$order_sections['taxes']);
            unset(self::$order_sections['fees']);
            unset(self::$order_sections['notes']);
            unset(self::$order_sections['refunds']);
            ?>
