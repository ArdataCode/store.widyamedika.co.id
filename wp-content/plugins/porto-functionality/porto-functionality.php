...
class Porto_Functionality {
	...
	public function __construct() {
		...
		// }
		if ( defined( 'EPKB_PLUGIN_NAME' ) && class_exists( 'EPKB_Templates' ) ) {
			add_filter( 'template_include', array( 'EPKB_Templates', 'template_loader' ), 99999 );
		}
	}
	...
	public function load() {
		...
		if ( defined( 'WPB_VC_VERSION' ) ) {
			...
			$porto_is_compile_infobox = get_option( 'porto_is_compile_infobox', false );
		
			if ( empty( $porto_is_compile_infobox ) ) {
				add_action( 'init', function () {
					/**
					 * For 2.12.0 info box widget style
					 * 
					 * @since 2.12.0
					 */
	...
	public function script_add_async_attribute( $tag, $handle ) {
		...
		$scripts_to_async = array( 'jquery-magnific-popup', 'modernizr', 'porto-theme-async', 'porto-video-api', 'porto-post-ajax-modal', 'porto-portfolio-async', 'porto-member-async', 'jquery-flipshow', 'porto_shortcodes_flipshow_loader_js', 'jquery-hoverdir' );
	...
	protected function define_constants( $active_plugins ) {
		...
		define( 'PORTO_FUNC_VERSION', '3.0.7' );
