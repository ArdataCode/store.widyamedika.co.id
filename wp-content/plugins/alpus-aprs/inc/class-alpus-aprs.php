    ...
    public function get_summary_from_openai( $contents, $type = false, $tags = array() ) {
        ...
        if ( false == $type ) {
            ...
            if ( 'pros' == $type || 'cons' == $type ) {
                ...
                $input_text = 'Please summarize below summaries in detail more than 5 sentences ' . ( empty( $lang_name ) ? '' : ( 'in language that its code is ' . $lang_name ) ) . ':\n';
            } else if ( 'tags' == $type ) {
                $input_text = 'Please choose tags from list that only contains in below Product Summary as JSON format without any description:\n' . implode( ', ', $tags ) . '.\n
